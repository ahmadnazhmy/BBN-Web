  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        setError('Gagal mengambil data order: ' + err.message);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4 py-8">
      <div className="flex flex-col md:flex-row gap-6 w-full max-w-5xl bg-white border border-gray-300 shadow-md rounded-xs p-6">
        <div className="w-full md:w-2/3 space-y-4">
          <h2 className="text-xl md:text-2xl font-bold text-center md:text-left">Pembayaran Ulang</h2>
          ...
          <form onSubmit={handleUpload} className="space-y-4 pt-4">
            <div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <span className="text-sm text-gray-600">{file ? file.name : 'Belum ada file dipilih'}</span>
              </div>
              <input id="upload" type="file" accept="image/*" onChange={(e) => setFile(e.target.files[0])} className="hidden" />
            </div>

            <button type="submit" disabled={uploading} className="w-full bg-blue-800 hover:bg-blue-900 text-white px-4 py-2 rounded-xs disabled:opacity-50">
            ...
            {order.items?.map((item, idx) => (
              <li key={`${item.product_id}-${idx}`} className="flex justify-between border-b pb-2 text-sm">
                <span className="flex items-center gap-x-2">
                  <span className="bg-blue-800 text-white rounded-xs w-6 h-6 flex items-center justify-center text-xs">{item.quantity}</span>
                  <span>
                    {item.product_name}
                    {item.type ? ` ${item.type}` : ''}
